...
<div>
  <p className="text-sm text-gray-700 mb-2">adminDashboard/fripProducts</p>
  <h1 className="text-2xl font-semibold mb-4">All Products</h1>

  <Link href="/adminDashboard/fripProducts/orders"><Button variant={"link"}>View Orders</Button></Link>


  <Card className="xl:col-span-4">
  <CardHeader className="px-4 sm:px-7">
    <div className="ml-0 sm:ml-5 mt-2">
      <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0"> {/* Flex container with space between items */}
        {/* Sorting select */}
        <Select
          value={sortOption}
        ...
        <Link href="/adminDashboard/fripProducts/addFripProduct">
          <Button
      ...
      <p className='text-gray-600 text-sm'>
        <span className='text-blue-600 font-medium'>Total Products:</span> {fripProducts.length}
      </p>
    </div>
  </CardHeader>
  <hr className="border-t border-gray-300 mb-5" /> {/* Add this line for the header line */}
  <CardContent>
    {fripProducts.length == 0 && (
      <>
    ...
    <>
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4'>
        {fripProducts.map((product, index) => (
