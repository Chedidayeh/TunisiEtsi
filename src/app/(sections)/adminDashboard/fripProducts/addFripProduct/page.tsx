...
const Page = () => {
  ...
  const [selectedSize, setSelectedSize] = useState<string>(""); // Changed to single string
  ...
  const handleSizeToggle = (size: string) => {
    setSelectedSize(size); // Only set the selected size
  };
...
const handleAddProducts = async () => {
    ...
    const uploadPromises = filteredFiles.map(file => uploadProduct(file));

    try {
      const uploadPaths = await Promise.all(uploadPromises);

      await saveInDb(uploadPaths, title , price , selectedSize , brand , category);
      ...
      setOpen(false)
                ...
                const uploadProduct = async (file : File) =>{
                  ...
                  try {
                    ...
                    const res = await fetch('/api/uploadFripProducts', {
                      method: 'POST',
                      body: data
                    })
  ...
  return (
    <>
      ...
      <p className="text-sm text-gray-700 mb-2">AdminDashboard/AddFripProduct</p>
