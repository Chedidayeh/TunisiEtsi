...
import { FripOrder, FripOrderItem, Order, OrderItem, OrderStatus, OrderType } from "@prisma/client"
...
const OrderData = ({ordersData}: DesignViewProps) => {
    ...
    const handleFilterChange = (value: string) => {
     ...
     // get order item code
     const [selecteditems, setItems] = useState<OrderItem[]>([]);
                ...
                <AlertDialogTrigger asChild>
                 ...
                 {isLoading && (
                  ...
                  <Carousel className="w-full max-w-xs">
                   <CarouselContent>
                   {selecteditems
                      ?.flatMap(item => item.capturedMockup || []) // Flatten capturedMockup arrays
                    ...
                    {selectedOrder.orderItems.map((item) => (
                      <li key={item.id} className="flex items-center justify-between">
                        <span className="text-muted-foreground">
                          {item.productTitle} <span>({item.productCategory}) ({item.productSize})</span>
                        </span>
